...
export const calculateCoverage = (items: CartItem[], products: any[]): number => {
...
export const validatePromoCode = (
  ...
  const promo = availableCodes.find(p => p.code.toLowerCase() === code.toLowerCase());
...
export const generateCartSummary = (items: CartItem[], products: any[]) => {
  const itemsWithDetails = items.map(item => {
    ...
    return {
      ...item,
      product,
      coverage: product ? parseInt(product.coverage?.match(/(\d+)/)?.[1] || '350') : 350,
    };
  });

  const totalItems = items.reduce((sum, item) => sum + item.quantity, 0);
  const totalCoverage = itemsWithDetails.reduce((sum, item) => sum + (item.coverage * item.quantity), 0);
