import { CheckoutFormData, OrderConfirmationData, ShippingAddress, ShippingOption, PromoCode } from '@/types/checkout';
import { CartItem, Product } from '@/types/product';
import { mockProducts } from '@/data/mock-products';
import { mockExteriorPaints } from '@/data/mock-exterior-paints';
import { Order, DashboardStats } from '@/types/account';
import { ConsultationPackage, ConsultationSlot, BookingFormData, PortfolioProject } from '@/types/consultation';
import { GalleryProject, ProjectSubmissionFormData, SocialMediaPost, Contest, ContestSubmissionFormData, CustomerStory } from '@/types/gallery';
import { CustomerReview, ReviewSubmissionFormData, ProfessionalEndorsement, CaseStudy } from '@/types/reviews';
import { NewsletterSubscription, NewsletterSubscriptionFormData, ContentArchiveItem } from '@/types/newsletter';
 ...
 */
export const mockValidateAddress = async (address: ShippingAddress): Promise<boolean> => {
      ...
      if (success) {
        ...
        const itemsWithDetails = cartItems.map(item => {
          ...
          return {
            ...
            price: item.price,
            image: color?.image || 'https://images.pexels.com/photos/6782371/pexels-photo-6782371.jpeg?auto=compress&cs=tinysrgb&w=800',
          };
        });

        const paymentMethodSummary = formData.paymentMethod.type === 'credit'
          ? `Credit Card (**** **** **** ${formData.paymentMethod.cardNumber?.slice(-4)})`
          : 'PayPal';
        ...
        resolve({
          ...
          mixingInstructionsLink: 'https://support.bolt.new/docs/paint-mixing-instructions',
        });
      } else {
        reject(new Error('Payment failed or an unexpected error occurred. Please try again.'));
      }
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([
        ...
        { code: 'FREESHIP', discount: 0, type: 'fixed', description: 'Free standard shipping' },
        { code: 'PAINT25', discount: 25, type: 'fixed', description: '$25 off orders over $200', minOrderAmount: 200 }
      ]);
    }, 200);
  });
};
 ...
 */
export const mockFetchConsultationPackages = async (): Promise<ConsultationPackage[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([
        {
          ...
          type: 'virtual',
          price: 75.00,
          description: 'A quick virtual session to get your color journey started. Perfect for small projects or initial ideas.',
          features: [
        ...
        },
        {
          ...
          type: 'in-home',
          price: 250.00,
          description: 'Comprehensive in-home consultation for a cohesive color scheme throughout your space.',
          features: [
        ...
        },
        {
          ...
          type: 'commercial',
          price: 500.00,
          description: 'Expert color strategy for commercial properties, focusing on brand identity and functionality.',
          features: [
 ...
 */
export const mockFetchAvailableSlots = async (date: string): Promise<ConsultationSlot[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
 ...
 */
export const mockBookConsultation = async (bookingData: BookingFormData): Promise<string> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const success = Math.random() > 0.1; // 90% success rate
      if (success) {
        console.log('Booking successful:', bookingData);
        resolve(`Your ${bookingData.serviceType} consultation for ${bookingData.date} at ${bookingData.time} has been confirmed!`);
      } else {
        reject(new Error('Failed to book consultation. The slot might be taken or an error occurred.'));
      }
    }, 1000);
  });
};
 ...
 */
export const mockFetchPortfolioProjects = async (): Promise<PortfolioProject[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([
        {
          ...
          client: 'Sarah & Tom D.',
          description: 'Transformed a dull living space into a vibrant, modern sanctuary with a cohesive color palette.',
          imageUrls: [
        ...
        },
        {
          ...
          description: 'Created an inspiring and productive environment for a tech startup, using colors that promote creativity and focus.',
          imageUrls: [
        ...
        },
        {
          ...
          client: 'Jane M.',
          description: 'Designed a serene and cozy bedroom atmosphere with soft, muted tones for ultimate relaxation.',
          imageUrls: [
...
const mockGalleryProjects: GalleryProject[] = [
  {
    ...
    description: 'Transformed my dull living room into a bright and inviting space using bold blues and a touch of sage green. The before and after is incredible!',
    beforeImageUrl: 'https://images.pexels.com/photos/1866149/pexels-photo-1866149.jpeg?auto=compress&cs=tinysrgb&w=800',
    afterImageUrl: 'https://images.pexels.com/photos/1866148/pexels-photo-1866148.jpeg?auto=compress&cs=tinysrgb&w=800',
    colorPalette: ['#4A90E2', '#5B7B7A', '#F5F5DC'],
    paintProductsUsed: [{ productId: 'premium-interior-paint-001', colorId: 'ocean-blue' }],
    likes: 125,
    views: 1500,
  ...
  },
  {
    ...
    description: 'Used a soft, warm cream to create a serene and cozy atmosphere in our master bedroom. Perfect for relaxation!',
    beforeImageUrl: 'https://images.pexels.com/photos/1743229/pexels-photo-1743229.jpeg?auto=compress&cs=tinysrgb&w=800',
    afterImageUrl: 'https://images.pexels.com/photos/1743230/pexels-photo-1743230.jpeg?auto=compress&cs=tinysrgb&w=800',
    colorPalette: ['#F5F5DC', '#D3D3D3'],
    paintProductsUsed: [{ productId: 'bedroom-sanctuary-003', colorId: 'warm-cream' }],
    likes: 88,
    views: 950,
  ...
  },
  {
    ...
    description: 'Revitalized a corporate office space with a professional and inspiring color scheme. Improved employee morale and productivity!',
    beforeImageUrl: 'https://images.pexels.com/photos/3862632/pexels-photo-3862632.jpeg?auto=compress&cs=tinysrgb&w=800',
    afterImageUrl: 'https://images.pexels.com/photos/3862633/pexels-photo-3862633.jpeg?auto=compress&cs=tinysrgb&w=800',
    colorPalette: ['#2C2C2C', '#E0E0E0', '#006994'],
    paintProductsUsed: [{ productId: 'commercial-pro', colorId: 'charcoal-gray' }],
    likes: 210,
    views: 2300,
  ...
  },
  {
    ...
    description: 'Gave my kitchen a fresh new look by repainting the cabinets. It feels so much brighter now!',
    beforeImageUrl: 'https://images.pexels.com/photos/3926542/pexels-photo-3926542.jpeg?auto=compress&cs=tinysrgb&w=800',
    afterImageUrl: 'https://images.pexels.com/photos/3926543/pexels-photo-3926543.jpeg?auto=compress&cs=tinysrgb&w=800',
    colorPalette: ['#FFFFFF', '#F5F5DC'],
    paintProductsUsed: [{ productId: 'kitchen-bath-002', colorId: 'pure-white' }],
    likes: 95,
    views: 1100,
 ...
 */
export const mockSubmitProject = async (formData: ProjectSubmissionFormData): Promise<GalleryProject> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const newProject: GalleryProject = {
        ...
        beforeImageUrl: formData.beforeImageFile ? URL.createObjectURL(formData.beforeImageFile) : undefined,
...
const mockSocialMediaPosts: SocialMediaPost[] = [
  {
    ...
    imageUrl: 'https://images.pexels.com/photos/6782371/pexels-photo-6782371.jpeg?auto=compress&cs=tinysrgb&w=800',
    caption: 'Loving this sage green in a cozy reading nook! #paintideas #homedecor',
    likes: 1230,
    comments: 45,
  ...
  },
  {
    ...
    caption: 'Our latest project: a modern living room with a stunning accent wall! What do you think?',
    likes: 890,
    comments: 70,
  ...
  },
  {
    ...
    imageUrl: 'https://images.pexels.com/photos/6782375/pexels-photo-6782375.jpeg?auto=compress&cs=tinysrgb&w=800',
    caption: 'Deep burgundy is making a comeback! Perfect for a dramatic dining room.',
    likes: 2100,
    comments: 120,
 ...
 */
export const mockFetchSocialMediaFeed = async (): Promise<SocialMediaPost[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
...
const mockContests: Contest[] = [
  {
    ...
    description: 'Show us your best summer home refresh project! Transform a room with a fresh coat of paint and win amazing prizes.',
    startDate: '2024-08-01',
    endDate: '2024-08-31',
    theme: 'Bright & Airy Spaces',
    prizes: ['Gift Card ($500)', 'Paint Supply Bundle', 'Featured on Social Media'],
  ...
  },
  {
    ...
    description: 'Unleash your creativity in your child\'s room! Submit your most imaginative and colorful kids\' room paint projects.',
 ...
 */
export const mockSubmitContestEntry = async (formData: ContestSubmissionFormData): Promise<string> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (Math.random() > 0.1) { // 90% success rate
        console.log('Contest entry submitted:', formData);
        resolve('Your contest entry has been submitted successfully! Good luck!');
      } else {
...
const mockCustomerStories: CustomerStory[] = [
  {
    ...
    imageUrl: 'https://images.pexels.com/photos/1866148/pexels-photo-1866148.jpeg?auto=compress&cs=tinysrgb&w=800',
  ...
  },
  {
    ...
    location: 'Seattle, WA',
    story: 'As first-time parents, decorating the nursery felt daunting. Your eco-friendly, low-VOC paint made us feel safe, and the soft lavender color created the perfect calming environment. It was surprisingly easy to apply, and we\'re so proud of the result. Our little one loves their new room!',
    imageUrl: 'https://images.pexels.com/photos/1743230/pexels-photo-1743230.jpeg?auto=compress&cs=tinysrgb&w=800',
...
const mockCustomerReviews: CustomerReview[] = [
  {
    ...
    businessResponse: {
      ...
      responseText: 'Thank you, Sarah! We are thrilled to hear you love our Premium Interior Paint. Your beautiful living room transformation is truly inspiring!',
  ...
  },
  {
    ...
    rating: 4,
    comment: 'Great eco-friendly option. No strong odors and safe for the kids. Took two coats on a dark wall, but overall very happy.',
    date: '2024-07-20',
    verifiedPurchase: true,
    photos: [],
    helpfulVotes: 8,
  ...
  },
  {
    ...
    rating: 5,
    comment: 'Absolutely stunning colors and finish. Worth every penny! My dining room looks like a masterpiece.',
    date: '2024-07-18',
    verifiedPurchase: true,
    photos: [
  ...
  },
  {
    ...
    rating: 4,
    comment: 'Perfect for our bathroom renovation. No mold issues after 6 months! The Spa Blue is very calming.',
    date: '2024-07-10',
    verifiedPurchase: true,
    photos: [],
    helpfulVotes: 10,
  ...
  },
  {
    ...
    rating: 3,
    comment: 'The paint color was a bit different than expected from the online swatch. Quality is decent though.',
    date: '2024-07-05',
 ...
 */
export const mockSubmitReview = async (formData: ReviewSubmissionFormData): Promise<CustomerReview> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const newReview: CustomerReview = {
        ...
        photos: formData.photos ? formData.photos.map(file => URL.createObjectURL(file)) : [],
        helpfulVotes: 0,
        unhelpfulVotes: 0,
      };
 ...
 */
export const mockVoteReviewHelpfulness = async (reviewId: string, type: 'helpful' | 'unhelpful'): Promise<{ helpfulVotes: number; unhelpfulVotes: number }> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (review) {
        ...
        resolve({ helpfulVotes: review.helpfulVotes, unhelpfulVotes: review.unhelpfulVotes });
      } else {
 ...
 */
export const mockSubmitBusinessResponse = async (reviewId: string, responseText: string): Promise<CustomerReview> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const review = mockCustomerReviews.find(r => r.id === reviewId);
      if (review) {
...
const mockProfessionalEndorsements: ProfessionalEndorsement[] = [
  {
    ...
    quote: 'The depth of color and durability of these paints are unmatched. My clients consistently rave about the results. A truly professional-grade product.',
    photoUrl: 'https://images.pexels.com/photos/3760303/pexels-photo-3760303.jpeg?auto=compress&cs=tinysrgb&w=400',
    date: '2024-05-10',
  },
  {
    id: 'prof-002',
    ...
    quote: 'I\'ve used countless brands over 30 years, and this paint consistently delivers superior coverage and a flawless finish. It saves me time and ensures client satisfaction.',
    photoUrl: 'https://images.pexels.com/photos/3760304/pexels-photo-3760304.jpeg?auto=compress&cs=tinysrgb&w=400',
    date: '2024-04-20',
  },
  {
    id: 'prof-003',
    ...
    quote: 'Their color palette is incredibly sophisticated, and the technical support is invaluable. I confidently recommend their products for both residential and commercial projects.',
    photoUrl: 'https://images.pexels.com/photos/3760305/pexels-photo-3760305.jpeg?auto=compress&cs=tinysrgb&w=400',
    date: '2024-06-01',
  },
];
 ...
 */
export const mockFetchProfessionalEndorsements = async (): Promise<ProfessionalEndorsement[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
...
const mockCaseStudies: CaseStudy[] = [
  {
    ...
    problem: 'Faded, peeling paint on a 100-year-old Victorian home, requiring historically accurate colors and durable exterior protection.',
    solution: 'Utilized specialized exterior paints with advanced weather resistance, meticulously color-matched to original historical swatches, and applied with expert techniques.',
    results: 'The home was restored to its original grandeur, with vibrant, long-lasting colors that enhanced its curb appeal and protected it from the elements. Client satisfaction was extremely high.',
    imageUrl: 'https://images.pexels.com/photos/164009/pexels-photo-164009.jpeg?auto=compress&cs=tinysrgb&w=800',
    fullStoryLink: '#',
  },
  {
    id: 'case-002',
    ...
    industry: 'Commercial',
    problem: 'A drab, uninspiring office environment leading to low employee morale and perceived lack of creativity.',
    solution: 'Conducted a comprehensive color consultation to select a palette that promotes focus, creativity, and collaboration, using low-VOC interior paints for a healthier workspace.',
    results: 'Employees reported increased satisfaction and productivity. The vibrant new environment reflected the company\'s innovative spirit and impressed visiting clients.',
    imageUrl: 'https://images.pexels.com/photos/3862633/pexels-photo-3862633.jpeg?auto=compress&cs=tinysrgb&w=800',
    fullStoryLink: '#',
  },
];
 ...
 */
export const mockSubscribeToNewsletter = async (formData: NewsletterSubscriptionFormData): Promise<NewsletterSubscription> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const existing = mockSubscriptions.find(s => s.email === formData.email);
      if (existing) {
 ...
 */
export const mockFetchSubscription = async (email: string): Promise<NewsletterSubscription | null> => {
  return new Promise((resolve) => {
    setTimeout(() => {
 ...
 */
export const mockUpdateSubscription = async (updatedSubscription: NewsletterSubscription): Promise<NewsletterSubscription> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const index = mockSubscriptions.findIndex(s => s.id === updatedSubscription.id);
      if (index !== -1) {
        mockSubscriptions[index] = { ...updatedSubscription, lastUpdated: new Date().toISOString() };
...
const mockContentArchive: ContentArchiveItem[] = [
  {
    ...
    description: 'Discover the colors and finishes dominating interior design this year.',
    publishDate: '2024-07-15',
    imageUrl: 'https://images.pexels.com/photos/6782372/pexels-photo-6782372.jpeg?auto=compress&cs=tinysrgb&w=800',
  ...
  },
  {
    ...
    description: 'Step-by-step instructions for a flawless cabinet transformation.',
    publishDate: '2024-07-01',
    imageUrl: 'https://images.pexels.com/photos/3926543/pexels-photo-3926543.jpeg?auto=compress&cs=tinysrgb&w=800',
  ...
  },
  {
    ...
    description: 'Learn about durable exterior paint options that withstand extreme weather.',
    publishDate: '2024-06-15',
    imageUrl: 'https://images.pexels.com/photos/164005/pexels-photo-164005.jpeg?auto=compress&cs=tinysrgb&w=800',
  ...
  },
  {
    ...
    description: 'Explore the calming and sophisticated appeal of sage green in various settings.',
 ...
 */
export const mockFetchContentArchive = async (filters?: { category?: string }): Promise<ContentArchiveItem[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
 ...
 * @returns A promise that resolves with an array of ContentArchiveItem.
 */
export const mockFetchRecommendations = async (email: string): Promise<ContentArchiveItem[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
...
const mockFAQData: FAQItem[] = [
  {
    ...
    answer: 'For living rooms, we recommend our Premium Interior Latex Paint. It offers excellent durability, washability, and a smooth finish, perfect for high-traffic areas. Eggshell or satin finishes are popular choices for their subtle sheen and easy maintenance.',
  ...
  },
  {
    ...
    answer: 'We offer a 30-day return policy for unopened, unused paint and supplies in their original condition. Custom-tinted paints are non-returnable. Please bring your original receipt for a full refund or exchange. For online orders, please contact customer service to arrange a return.',
  ...
  },
  {
    ...
    answer: 'Standard shipping typically takes 5-7 business days. Expedited shipping is available for 2-3 business days, and overnight shipping delivers on the next business day. Delivery times may vary based on your location and product availability.',
    category: 'shipping_returns',
  ...
  },
  {
    ...
    answer: 'Several factors can influence how paint color appears. Lighting (natural vs. artificial, warm vs. cool), surrounding colors in the room, and the texture of the wall can all affect perception. Always test a sample on your wall and observe it throughout the day under different lighting conditions before committing to the entire room.',
    category: 'color_matching',
 ...
 */
export const mockFetchFAQs = async (query?: string, category?: FAQCategory): Promise<FAQItem[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
 ...
 * @returns A promise that resolves with the updated vote counts.
 */
export const mockVoteFAQ = async (faqId: string, type: 'helpful' | 'unhelpful'): Promise<{ helpfulVotes: number; unhelpfulVotes: number }> => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (faq) {
        ...
        resolve({ helpfulVotes: faq.helpfulVotes, unhelpfulVotes: faq.unhelpfulVotes });
      } else {
