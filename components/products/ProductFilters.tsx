...
const FilterSection: React.FC<FilterSectionProps> = ({
	...
	return (
		<div className="border-b border-gray-200 pb-6 mb-6 last:border-b-0 last:pb-0 last:mb-0">
			...
			>
				<h3 className="font-semibold text-ds-primary-charcoal">{title}</h3>
				{isOpen ? (
...
export const ProductFilters: React.FC<ProductFiltersProps> = ({
			...
			products.flatMap((p) =>
				p.colors.map((c) => {
					...
					if (
						...
						return "neutral";
					if (name.includes("brown") || name.includes("mustard"))
						return "brown";
	...
	const finishTypes = Array.from(
		new Set(
			products.flatMap((p) => p.finishes.map((f) => f.name.toLowerCase()))
		)
	);
				...
				{colorFamilies.map((family) => (
					...
					>
						<input
							...
							onChange={(e) =>
								handleCheckboxChange("colorFamilies", family, e.target.checked)
							}
							...
							(
							{
								products.filter((p) =>
									p.colors.some((c) => {
										const name = c.name.toLowerCase();
										if (family === "green")
											return name.includes("green") || name.includes("sage");
										if (family === "gray")
											return (
												name.includes("gray") ||
												name.includes("grey") ||
										...
										if (family === "brown")
											return name.includes("brown") || name.includes("mustard");
				...
				{finishTypes.map((finish) => (
					...
					>
						<input
							...
							onChange={(e) =>
								handleCheckboxChange("finishTypes", finish, e.target.checked)
							}
							className="w-4 h-4 text-ds-primary-sage border-gray-300 rounded focus:ring-ds-primary-sage focus:ring-2"
						/>
						...
						<span className="text-xs text-gray-500">
							...
							{
								products.filter((p) =>
									p.finishes.some((f) => f.name.toLowerCase() === finish)
								).length
							}
							)
						</span>
				...
				{roomTypes.map((room) => (
					...
					>
						<input
							...
							onChange={(e) =>
								handleCheckboxChange("roomTypes", room, e.target.checked)
							}
							className="w-4 h-4 text-ds-primary-sage border-gray-300 rounded focus:ring-ds-primary-sage focus:ring-2"
						/>
