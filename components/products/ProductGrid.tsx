              ...
              <div className="flex-1 p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h3 className="text-xl font-semibold text-[#2C2C2C] mb-2 group-hover:text-[#5B7B7A] transition-colors duration-200">
                      {product.name}
                    </h3>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    disabled={isInComparison(product.id) || compareProducts.length >= 3}
                    className={cn(
                      "p-2 rounded-lg border-2 transition-all duration-200",
                      isInComparison(product.id)
                        ? "border-[#5B7B7A] bg-[#5B7B7A]/10 text-[#5B7B7A]"
                        : "border-gray-300 text-gray-600 hover:border-[#5B7B7A] hover:text-[#5B7B7A]"
                    )}
                    title={isInComparison(product.id) ? "Added to comparison" : "Add to comparison"}
                  >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {products.map((product) => (
        <Link
          key={product.id}
          href={`/product/${product.id}`}
          className="group bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg transition-all duration-200 border border-gray-100"
        >
          {/* Product Image */}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isInComparison(product.id)
                      ...
                      : "bg-white text-gray-600 hover:bg-[#5B7B7A] hover:text-white"
                  )}
                  title={isInComparison(product.id) ? "Added to comparison" : "Add to comparison"}
                >
                  <Plus className="w-4 h-4" />
                </button>
                <button className="p-2 bg-white text-gray-600 rounded-full shadow-lg hover:bg-red-50 hover:text-red-500 transition-all duration-200">
                  <Heart className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <button
                  ...
                  className={cn(
                    ...
                    product.inStock
                      ? "bg-white text-[#5B7B7A] hover:bg-[#5B7B7A] hover:text-white"
                      : "bg-gray-300 text-gray-500 cursor-not-allowed"
                  )}
                >
                  <ShoppingCart className="w-4 h-4" />
          ...
          <div className="p-4">
            ...
            <h3 className="font-semibold text-[#2C2C2C] mb-2 group-hover:text-[#5B7B7A] transition-colors duration-200 line-clamp-2">
              {product.name}
            </h3>
