...
import { Clock, Paintbrush, Shield, Droplets, Thermometer, Ruler, Palette, CheckCircle } from 'lucide-react';
...
export const ProductSpecs: React.FC<ProductSpecsProps> = ({ product }) => {
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-[#2C2C2C]">Specifications</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {primarySpecs.map((spec, index) => (
          ...
          >
            <div className="text-[#5B7B7A] flex-shrink-0">
              {spec.icon}
            </div>
            <div>
              <p className="font-medium text-[#2C2C2C] text-sm">{spec.label}</p>
        ...
        {detailedSpecs.map((section, sectionIndex) => (
          <div key={sectionIndex} className="border border-gray-200 rounded-lg p-6">
            <h4 className="font-semibold text-[#2C2C2C] mb-4 flex items-center space-x-2">
              {section.category === 'Performance' && <CheckCircle className="w-5 h-5 text-[#5B7B7A]" />}
              {section.category === 'Technical' && <Ruler className="w-5 h-5 text-[#5B7B7A]" />}
              {section.category === 'Application Conditions' && <Thermometer className="w-5 h-5 text-[#5B7B7A]" />}
              {section.category === 'Coverage & Consumption' && <Palette className="w-5 h-5 text-[#5B7B7A]" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {section.specs.map((spec, specIndex) => (
                <div key={specIndex} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                  ...
                  <span className="text-sm font-medium text-[#2C2C2C]">{spec.value}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-[#F5F5DC]/30 p-4 rounded-lg">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p className="font-medium text-[#2C2C2C]">Brand</p>
            <p className="text-gray-600">{product.brand}</p>
          </div>
          <div>
            <p className="font-medium text-[#2C2C2C]">SKU</p>
