        ...
        {finishes.map((finish) => {
          ...
          return (
            <button
              ...
              aria-label={`Select ${finish.name} finish - $${totalPrice.toFixed(2)}`}
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <span className="font-semibold text-[#2C2C2C]">{finish.name}</span>
                  {isPremium && (
                    <span className="px-2 py-1 bg-[#C4A57B] text-white text-xs font-medium rounded-full">
                      Premium
                    </span>
                  )}
                </div>
                <div className="text-right">
                  <p className="font-bold text-[#2C2C2C]">${totalPrice.toFixed(2)}</p>
                  {finish.price > 0 && (
                    <p className="text-xs text-gray-500">+${finish.price.toFixed(2)}</p>
                  )}
                </div>
              </div>
              ...
              {finish.id === selectedFinish.id && (
                <div className="absolute inset-0 rounded-lg ring-2 ring-[#5B7B7A]/20 pointer-events-none" />
              )}
            </button>
          );
        })}
