...
import { Play, Clock, CheckCircle, AlertTriangle, Brush, Droplets, Thermometer } from 'lucide-react';
...
export const ApplicationGuide: React.FC<ApplicationGuideProps> = ({ productName }) => {
  const [activeStep, setActiveStep] = useState(0);
  const [activeGuide, setActiveGuide] = useState<'preparation' | 'application' | 'cleanup'>('preparation');
  ...
  const preparationSteps = [
    ...
    },
    {
      ...
      warning: 'Never skip primer on bare surfaces or when making dramatic color changes.'
    },
    {
  ...
  const getCurrentSteps = () => {
    switch (activeGuide) {
      case 'preparation': return preparationSteps;
      case 'application': return applicationSteps;
      case 'cleanup': return cleanupSteps;
      default: return preparationSteps;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-[#2C2C2C]">Application Guide</h3>
      ...
      <div className="border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            <div className="w-8 h-8 bg-[#5B7B7A] text-white rounded-full flex items-center justify-center font-bold text-sm">
              {activeStep + 1}
            </div>
            <div>
              <h4 className="font-semibold text-[#2C2C2C]">{currentStep.title}</h4>
              <div className="flex items-center space-x-2 text-sm text-gray-600">
        ...
        <div className="space-y-3 mb-4">
          {currentStep.details.map((detail, index) => (
            <div key={index} className="flex items-start space-x-3">
              <CheckCircle className="w-5 h-5 text-[#5B7B7A] mt-0.5 flex-shrink-0" />
        ...
        <div className="space-y-3">
          <div className="bg-[#5B7B7A]/5 border border-[#5B7B7A]/20 rounded-lg p-3">
            <div className="flex items-start space-x-2">
              <CheckCircle className="w-4 h-4 text-[#5B7B7A] mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-sm font-medium text-[#5B7B7A]">Pro Tip</p>
          ...
          <button
            ...
            className={cn(
              ...
              activeStep === currentSteps.length - 1
                ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                : "bg-[#5B7B7A] text-white hover:bg-[#5B7B7A]/90"
            )}
          >
            Next Step
          </button>
      ...
      <div className="bg-[#F5F5DC]/30 border border-[#C4A57B]/20 rounded-lg p-6">
        <h4 className="font-semibold text-[#2C2C2C] mb-4 flex items-center space-x-2">
          <Brush className="w-5 h-5 text-[#5B7B7A]" />
          <span>Required Tools & Materials</span>
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <h5 className="font-medium text-[#2C2C2C] mb-2">Tools</h5>
          ...
          <div>
            <h5 className="font-medium text-[#2C2C2C] mb-2">Materials</h5>
            <ul className="space-y-1 text-sm text-gray-700">
              <li>• {productName}</li>
