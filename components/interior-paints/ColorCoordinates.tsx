...
export const ColorCoordinates: React.FC<ColorCoordinatesProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'schemes' | 'custom' | 'trends'>('schemes');
      ...
      <div className="bg-white rounded-lg max-w-6xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            <Palette className="w-6 h-6 text-[#5B7B7A]" />
            <div>
              <h2 className="text-2xl font-bold text-[#2C2C2C]">Color Coordinates</h2>
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              className="p-2 text-gray-600 hover:text-[#5B7B7A] transition-colors duration-200"
              title="Generate random scheme"
            >
              <RefreshCw className="w-5 h-5" />
            </button>
            <button className="p-2 text-gray-600 hover:text-[#5B7B7A] transition-colors duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {colorSchemes.map((scheme) => (
                    ...
                    >
                      ...
                      <h4 className="font-semibold text-[#2C2C2C] mb-1">{scheme.name}</h4>
                      <p className="text-sm text-gray-600 mb-2">{scheme.description}</p>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-[#5B7B7A] font-medium">{scheme.mood}</span>
                        <span className="text-gray-500">{scheme.rooms.length} rooms</span>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                {selectedScheme ? (
                  <div className="bg-gray-50 rounded-lg p-6">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-4">
                      ...
                      <div
                        className="h-20 rounded-lg border border-gray-200 flex items-end p-2"
                        style={{ backgroundColor: selectedScheme.colors.secondary }}
                      >
                        <span className="text-xs font-medium bg-white/90 px-2 py-1 rounded">
                          Secondary
                        </span>
                      ...
                      <div
                        className="h-20 rounded-lg border border-gray-200 flex items-end p-2"
                        style={{ backgroundColor: selectedScheme.colors.neutral }}
                      >
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex justify-between text-sm">
                        <span>Primary:</span>
                        <span className="font-mono">{selectedScheme.colors.primary}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Secondary:</span>
                        <span className="font-mono">{selectedScheme.colors.secondary}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Accent:</span>
                        <span className="font-mono">{selectedScheme.colors.accent}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>Neutral:</span>
                        <span className="font-mono">{selectedScheme.colors.neutral}</span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div>
                        <h5 className="font-medium text-[#2C2C2C] mb-1">Mood</h5>
                        <p className="text-sm text-gray-600">{selectedScheme.mood}</p>
                      </div>
                      <div>
                        <h5 className="font-medium text-[#2C2C2C] mb-1">Best for</h5>
                        <div className="flex flex-wrap gap-1">
                          {selectedScheme.rooms.map((room, index) => (
                            <span
                              key={index}
                              className="text-xs bg-[#5B7B7A]/10 text-[#5B7B7A] px-2 py-1 rounded-full"
                            >
                              {room}
                            </span>
                          ))}
                    ...
                    <button className="w-full mt-4 py-2 bg-[#5B7B7A] text-white rounded-lg hover:bg-[#5B7B7A]/90 transition-colors duration-200">
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 text-center">
                    ...
                    <p className="text-sm text-gray-600">
                      Choose a scheme to see detailed color information and matching paint options.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {trendingColors.map((color, index) => (
                  <div key={index} className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    />
                    <h4 className="font-semibold text-[#2C2C2C] mb-1">{color.name}</h4>
                    <p className="text-sm text-gray-600 mb-2">{color.hex}</p>
                    <span className="text-xs bg-[#5B7B7A]/10 text-[#5B7B7A] px-2 py-1 rounded-full">
                      {color.trend}
                    </span>
                  </div>
                ))}
          ...
          {activeTab === 'custom' && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                Create your own custom color palette with our advanced color picker tools.
              </p>
              <button className="px-6 py-3 bg-[#5B7B7A] text-white rounded-lg hover:bg-[#5B7B7A]/90 transition-colors duration-200">
                Launch Color Picker
              </button>
            </div>
          )}
