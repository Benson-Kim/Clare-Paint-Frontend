...
export const PrimerCompatibility: React.FC = () => {
  ...
  const primers: PrimerInfo[] = [
    {
      ...
      type: 'Latex-Based',
      description: 'Superior coverage primer that blocks stains and provides excellent adhesion for interior surfaces.',
      bestFor: ['Dramatic color changes', 'Stain blocking', 'New drywall'],
      coverage: '350-400 sq ft per gallon',
      dryTime: '2-4 hours',
      price: 34.99,
      features: ['Stain blocking', 'High hide formula', 'Low odor', 'Quick dry'],
      compatibility: {
        surfaces: ['Drywall', 'Wood', 'Metal', 'Previously painted surfaces'],
        paints: ['All interior latex paints', 'Eggshell', 'Satin', 'Semi-gloss']
      }
    },
    {
      id: 'bonding-primer',
      name: 'Bonding Primer',
      type: 'Acrylic-Based',
      description: 'Advanced adhesion primer for challenging surfaces that typically require sanding.',
      bestFor: ['Glossy surfaces', 'Laminate', 'Tile', 'Glass'],
      coverage: '300-350 sq ft per gallon',
      dryTime: '1-2 hours',
      price: 42.99,
      features: ['Superior adhesion', 'No sanding required', 'Multi-surface', 'Fast dry'],
      compatibility: {
        surfaces: ['Glossy paint', 'Laminate', 'Ceramic tile', 'Glass', 'Metal'],
    ...
    },
    {
      ...
      type: 'Oil-Based',
      description: 'Maximum stain blocking power for the toughest stains including water damage, smoke, and crayon marks.',
      bestFor: ['Water stains', 'Smoke damage', 'Crayon marks', 'Tannin bleeding'],
      coverage: '350-400 sq ft per gallon',
      dryTime: '4-6 hours',
      price: 39.99,
      features: ['Maximum stain blocking', 'Odor sealing', 'Tannin blocking', 'Durable'],
      compatibility: {
        surfaces: ['All interior surfaces', 'Damaged drywall', 'Wood with stains'],
    ...
    },
    {
      ...
      type: 'Zero-VOC',
      description: 'Environmentally safe primer with zero harmful emissions, perfect for sensitive environments.',
      bestFor: ['Nurseries', 'Bedrooms', 'Eco-conscious projects'],
      coverage: '300-350 sq ft per gallon',
      dryTime: '2-3 hours',
      price: 37.99,
      features: ['Zero-VOC', 'Low odor', 'Safe for children', 'GREENGUARD certified'],
      compatibility: {
  ...
  return (
    <div className="mt-16 bg-[#F5F5DC]/20 border border-[#C4A57B]/20 rounded-lg p-8">
      <div className="text-center mb-8">
        <div className="flex items-center justify-center space-x-3 mb-4">
          <Shield className="w-8 h-8 text-[#5B7B7A]" />
          <h2 className="text-2xl md:text-3xl font-bold text-[#2C2C2C]">
            Primer Compatibility Guide
          </h2>
        </div>
        <p className="text-gray-700 max-w-3xl mx-auto">
          Ensure perfect paint adhesion and color accuracy with our comprehensive primer selection guide. 
          The right primer makes all the difference in your paint project's success.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          >
            <div className="flex items-center space-x-3">
              <Info className="w-5 h-5 text-[#5B7B7A]" />
          ...
          {expandedSection === 'overview' && (
            <div className="px-6 pb-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-[#2C2C2C] mb-3 flex items-center space-x-2">
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          >
            <div className="flex items-center space-x-3">
              <Shield className="w-5 h-5 text-[#5B7B7A]" />
              <h3 className="text-lg font-semibold text-[#2C2C2C]">
                Recommended Primers
              </h3>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {primers.map((primer) => (
                  <div
                    ...
                    onClick={() => setSelectedPrimer(selectedPrimer === primer.id ? null : primer.id)}
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-semibold text-[#2C2C2C]">{primer.name}</h4>
                      <span className="text-lg font-bold text-[#5B7B7A]">
                        ${primer.price.toFixed(2)}
                      </span>
                    </div>
                    
                    <div className="mb-3">
                      <span className="text-xs bg-[#C4A57B]/20 text-[#8B4513] px-2 py-1 rounded-full">
                        {primer.type}
                      </span>
                    </div>
                    
                    <p className="text-sm text-gray-600 mb-3">{primer.description}</p>
                    ...
                    {selectedPrimer === primer.id && (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="space-y-3">
                          <div>
                            <h5 className="text-sm font-medium text-[#2C2C2C] mb-1">Features</h5>
                            <div className="flex flex-wrap gap-1">
                              {primer.features.map((feature, index) => (
                                <span
                                  key={index}
                                  className="text-xs bg-[#5B7B7A]/10 text-[#5B7B7A] px-2 py-1 rounded"
                                >
                                  {feature}
                                </span>
                              ))}
                          ...
                          <div>
                            <h5 className="text-sm font-medium text-[#2C2C2C] mb-1">Compatible Surfaces</h5>
                            <p className="text-xs text-gray-600">
                              {primer.compatibility.surfaces.join(', ')}
                            </p>
                          </div>
                          
                          <button className="w-full py-2 bg-[#5B7B7A] text-white rounded-lg hover:bg-[#5B7B7A]/90 transition-colors duration-200 text-sm font-medium">
                            Add to Cart
                          </button>
                        </div>
                      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-3 font-semibold text-[#2C2C2C]">Surface Type</th>
                      <th className="text-left py-3 font-semibold text-[#2C2C2C]">Recommended Primer</th>
                      <th className="text-left py-3 font-semibold text-[#2C2C2C]">Required?</th>
                      <th className="text-left py-3 font-semibold text-[#2C2C2C]">Reason</th>
                    </tr>
                  </thead>
                  <tbody>
                    {compatibilityMatrix.map((item, index) => (
                      <tr key={index} className="border-b border-gray-100">
                        <td className="py-3 font-medium text-gray-700">{item.surface}</td>
                        <td className="py-3 text-[#5B7B7A]">{item.recommended}</td>
                        <td className="py-3">
                          {item.required ? (
      ...
      <div className="mt-8 text-center">
        <div className="bg-[#5B7B7A]/10 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-[#2C2C2C] mb-2">
            Need Help Choosing the Right Primer?
          </h3>
          <p className="text-gray-600 mb-4">
            Our paint experts are here to help you select the perfect primer for your project.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button className="px-6 py-3 bg-[#5B7B7A] text-white rounded-lg hover:bg-[#5B7B7A]/90 transition-colors duration-200 font-medium">
              Get Expert Consultation
            </button>
            <button className="px-6 py-3 border border-[#5B7B7A] text-[#5B7B7A] rounded-lg hover:bg-[#5B7B7A]/5 transition-colors duration-200 font-medium">
              Shop All Primers
            </button>
          </div>
        </div>
